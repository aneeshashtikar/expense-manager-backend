...
public interface TransactionRepository extends CrudRepository<Transactions, Long> {
    ...
    List<Object[]> getExpenses(@Param("no") String no);

    @Query(value = "select sum(amount) from transactions where  user_id = :no and category_id = 7", nativeQuery = true)
    Long getIncomeTotal(@Param("no") String no);
}
